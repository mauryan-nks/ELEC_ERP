...
<div class="ms-card ms-spacer-top">
    ...
    <div class="ms-invoice-controls">
        <div class="ms-toggle-block">
            ...
            <div class="ms-field ms-gst-mode-field">
                <label>GST price mode</label>
                <?php $gstModeDefault = ($shop['invoice_default_gst_mode'] ?? 'inclusive') === 'exclusive' ? 'exclusive' : 'inclusive'; ?>
                <div class="ms-radio-card-grid" role="radiogroup" aria-label="GST price mode">
                    <label class="ms-radio-card <?= $gstModeDefault === 'inclusive' ? 'is-selected' : '' ?>">
                        <input type="radio" name="gst_mode" value="inclusive" id="gstModeInclusive" <?= $gstModeDefault === 'inclusive' ? 'checked' : '' ?>>
                        <span class="ms-radio-mark"></span>
                        <span class="ms-radio-copy"><strong>Price includes GST</strong><small>The entered price already includes GST.</small></span>
                    </label>
                    <label class="ms-radio-card <?= $gstModeDefault === 'exclusive' ? 'is-selected' : '' ?>">
                        <input type="radio" name="gst_mode" value="exclusive" id="gstModeExclusive" <?= $gstModeDefault === 'exclusive' ? 'checked' : '' ?>>
                        <span class="ms-radio-mark"></span>
                        <span class="ms-radio-copy"><strong>Price excludes GST</strong><small>GST will be added to the entered price.</small></span>
                    </label>
                </div>
                <div class="ms-help">Choose how the product price should be treated when calculating GST. Inclusive is the default.</div>
            </div>
        </div>
        <div class="ms-toggle-block">
...
<dialog id="barcodeScannerDialog"><div class="ms-dialog-body"><div class="ms-dialog-head"><h3>Scan QR / barcode</h3><button type="button" class="ms-btn ms-btn-secondary is-sm" id="closeBarcodeScanner">Close</button></div><p class="ms-muted">Focus a product, IMEI, serial, or unique-ID field, then scan.</p><video id="barcodeScannerVideo" autoplay playsinline muted style="display:block;width:100%;max-height:360px;background:#111;border-radius:10px"></video><p id="barcodeScannerStatus" class="ms-muted"></p></div></dialog><button type="button" class="ms-btn ms-btn-secondary ms-spacer-top" id="openBarcodeScanner">Scan QR / barcode with camera</button>
...
function getGstMode(){return document.querySelector('input[name="gst_mode"]:checked')?.value==='exclusive'?'exclusive':'inclusive'}
function num(v){const n=parseFloat(v);return Number.isFinite(n)?n:0}
function money(v){return '₹'+Math.max(0,v).toFixed(2)}
function updateInvoicePreview(){let subtotal=0,lineDiscount=0,net=0;const rows=[...document.querySelectorAll('.sale-row')];rows.forEach(row=>{const q=num(row.querySelector('.qty')?.value),p=num(row.querySelector('.price')?.value),base=q*p,disc=discountEnabled.checked?Math.min(Math.max(0,num(row.querySelector('.discount')?.value)),base):0;subtotal+=base;lineDiscount+=disc;net+=Math.max(0,base-disc)});let overall=0;const type=document.getElementById('overallDiscountType').value,val=Math.max(0,num(document.getElementById('overallDiscountValue').value));if(discountEnabled.checked&&type==='amount')overall=Math.min(val,net);if(discountEnabled.checked&&type==='percent')overall=Math.min(100,val)*net/100;const ratio=net>0?overall/net:0;let tax=0;rows.forEach(row=>{const q=num(row.querySelector('.qty')?.value),p=num(row.querySelector('.price')?.value),base=q*p,disc=discountEnabled.checked?Math.min(Math.max(0,num(row.querySelector('.discount')?.value)),base):0,taxRate=gstEnabled.checked?Math.max(0,num(row.querySelector('.tax')?.value)):0,taxable=Math.max(0,(base-disc)*(1-ratio));if(gstEnabled.checked&&taxRate>0){tax+=(getGstMode()==='inclusive')?taxable-(taxable/(1+taxRate/100)):taxable*taxRate/100}});const grand=getGstMode()==='inclusive'?subtotal-lineDiscount-overall:subtotal-lineDiscount-overall+tax;document.getElementById('saleSubtotalPreview').textContent=money(subtotal);document.getElementById('saleLineDiscountPreview').textContent='− '+money(lineDiscount);document.getElementById('saleOverallDiscountPreview').textContent='− '+money(overall);document.getElementById('saleTaxPreview').textContent=money(tax);document.getElementById('saleGrandPreview').textContent=money(grand)}
gstEnabled.addEventListener('change',syncTaxDiscountUI);document.querySelectorAll('input[name="gst_mode"]').forEach(el=>el.addEventListener('change',()=>{document.querySelectorAll('.ms-radio-card').forEach(card=>card.classList.toggle('is-selected',card.querySelector('input')?.checked));updateInvoicePreview()}));discountEnabled.addEventListener('change',syncTaxDiscountUI);document.getElementById('overallDiscountType').addEventListener('change',updateInvoicePreview);document.getElementById('overallDiscountValue').addEventListener('input',updateInvoicePreview);
...
let barcodeTarget=null,barcodeStream=null,barcodeTimer=null;const barcodeDialog=document.getElementById('barcodeScannerDialog'),barcodeVideo=document.getElementById('barcodeScannerVideo'),barcodeStatus=document.getElementById('barcodeScannerStatus');document.addEventListener('focusin',e=>{if(e.target.matches('.product-select,.imei1,.imei2,.serial-no,.unique-id'))barcodeTarget=e.target});function stopBarcodeScanner(){clearInterval(barcodeTimer);barcodeTimer=null;if(barcodeStream)barcodeStream.getTracks().forEach(t=>t.stop());barcodeStream=null;barcodeVideo.srcObject=null;if(barcodeDialog.open)barcodeDialog.close()}async function startBarcodeScanner(){if(!('BarcodeDetector'in window)){shopToast('Camera scanning is not supported in this browser. Use Chrome on HTTPS.','error');return}if(!barcodeTarget)barcodeTarget=document.querySelector('.unique-id,.serial-no,.product-select');if(!barcodeTarget){shopToast('Add a sale row first.','error');return}barcodeDialog.showModal();barcodeStatus.textContent='Starting camera…';try{barcodeStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}}});barcodeVideo.srcObject=barcodeStream;await barcodeVideo.play();const detector=new BarcodeDetector({formats:['qr_code','code_128','code_39','ean_13','ean_8','upc_a','upc_e','itf']});barcodeStatus.textContent='Point the camera at a QR code or barcode.';barcodeTimer=setInterval(async()=>{const codes=await detector.detect(barcodeVideo);if(!codes.length)return;const value=codes[0].rawValue;if(barcodeTarget.tagName==='SELECT'){const option=[...barcodeTarget.options].find(o=>o.text.toLowerCase().includes(value.toLowerCase()));if(option){barcodeTarget.value=option.value;barcodeTarget.dispatchEvent(new Event('change',{bubbles:true}))}else shopToast('No product matches scanned code.','error')}else{barcodeTarget.value=value;barcodeTarget.dispatchEvent(new Event('input',{bubbles:true}))}stopBarcodeScanner()},350)}catch(e){barcodeStatus.textContent='Camera access was denied or is unavailable.'}}document.getElementById('openBarcodeScanner').onclick=startBarcodeScanner;document.getElementById('closeBarcodeScanner').onclick=stopBarcodeScanner;barcodeDialog.addEventListener('close',stopBarcodeScanner);
